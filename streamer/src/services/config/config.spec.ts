...
test('bootstrapConfig', async () => {
	...
	const [dbChain, dbChainType] = await Promise.all(
		[configService.getConfigValueFromDB('chain'), configService.getConfigValueFromDB('chain_type')]
	)
	...
	await expect(configService.bootstrapConfig())
		.rejects
		.toThrow(`Node "system.chainType" not compare to saved type: "${dbChainType}" and "${anotherChainType}`)

	await configService.updateConfigValueInDB('chain', anotherChain)
	await expect(configService.bootstrapConfig())
		.rejects
		.toThrow(`Node "system.chain" not compare to saved type: "${dbChain}" and "${anotherChain}"`)
})
