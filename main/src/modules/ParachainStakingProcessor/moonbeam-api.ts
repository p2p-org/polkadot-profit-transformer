...
export const reward = async (api: ApiPromise, nowBlockNumber: number): Promise<{round:any, stakedValue: any}> => {
  console.log("---------------------------------")
  const latestBlock = await api.rpc.chain.getBlock();
  const latestBlockHash = latestBlock.block.hash;
  const latestBlockNumber = latestBlock.block.header.number.toNumber();
  const latestRound:any = await (await api.at(latestBlock.block.hash)).query.parachainStaking.round();
  const nowBlockHash = await api.rpc.chain.getBlockHash(nowBlockNumber);
  const nowRound: any = await (await api.at(nowBlockHash)).query.parachainStaking.round();
  ...
  while (true) {
    ...
    const round: any = await (await api.at(blockHash)).query.parachainStaking.round();
    if (
  ...
  const apiAtOriginal:any = await api.at(originalRoundPriorBlockHash);
  ...
  const collatorCommissionRate: any =
...
/*
  ...
  */
  ...
  for await (const i of new Array(maxRoundChecks).keys()) {
    ...
    const rewarded:any = await getRewardedFromEventsAtBlock(
      api,
      specVersion,
      blockNumber,
      delegators,
...
async function getCollatorsAndDelegators(apiAtOriginal: ApiPromise, apiAtPriorRewarded: any, roundNumber: number): Promise<{
...
async function fixDelegatorsStakeQueue (
  ...
  return new Promise(async (res, rej) => {
    const processDelegators = async (delegatorId: string, cb: any): Promise<void> => {
      const zerodelegator2: any = await api.query.parachainStaking.delegatorState(delegatorId);
