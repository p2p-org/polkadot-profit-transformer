...
export const processDemocracyReferendaRemoveVoteExtrinsic = async (
  ...
  const removeVoteExtrinsic = signedBlock.block.extrinsics.find(
    (ext) => ext.method.section === 'democracy' && ext.method.method === 'removeVote',
  )

  if (!removeVoteExtrinsic) {
    logger.warn('no democracy.removeVote extrinsic found in block ' + extrinsic.block_id)
    return
  }
  const referendumIndex = <number>(<unknown>removeVoteExtrinsic.args[0])
