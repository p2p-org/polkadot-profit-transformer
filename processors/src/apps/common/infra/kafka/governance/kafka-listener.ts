...
export const KafkaListenerFactory = (processor: GovernanceProcessor, logger: Logger, environment: Environment) => {
          ...
          try {
            switch (topic) {
              case environment.KAFKA_PREFIX + '_GOVERNANCE_DATA':
                if (!environment.PROCESS_EVENTS) return
              ...
              case environment.KAFKA_PREFIX + '_EXTRINSICS_DATA':
                if (!environment.PROCESS_EXTRINSICS) return
